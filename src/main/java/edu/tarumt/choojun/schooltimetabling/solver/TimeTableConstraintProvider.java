package edu.tarumt.choojun.schooltimetabling.solver;
...
public class TimeTableConstraintProvider implements ConstraintProvider
{
	...
	Constraint studentGroupSubjectVariety(ConstraintFactory constraintFactory)
	{
		...
		return constraintFactory.forEach(Lesson.class).join(Lesson.class, Joiners.equal(Lesson::getSubject),
				...
				.filter((lesson1, lesson2) ->
				{
					Duration between = Duration.between(lesson1.getTimeslot().getEndTime(),
							lesson2.getTimeslot().getStartTime());
					return !between.isNegative() && between.compareTo(Duration.ofMinutes(30)) <= 0;
				}).penalize(HardSoftScore.ONE_SOFT).asConstraint("Student group subject variety");
	}
